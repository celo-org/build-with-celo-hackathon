...
/**
 * @title SafeCall
 * @dev Wrappers around IAccount and ERC20 operations that throw on failure (when the token
...
library SafeCall {
    ...
    function safeWithdrawRouterOnly(
        ...
        _callOptionalReturnAccount(
          account, 
          abi.encodeWithSelector(
            account.withdrawNativeOnlyRouter.selector, 
            token, 
            to,
            feeTo,
            amount,
    ...
    function safeClearSubscrition(
        ...
        _callOptionalReturnAccount(account, abi.encodeWithSelector(
          account.clearSubscrition.selector,
          poolId
        )
      );
    }
