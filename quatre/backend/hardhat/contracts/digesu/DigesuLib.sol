...
library DigesuLib {
  ...
  function _getPools(Data storage self) public view returns (ICommon.Pool[] memory) {
    return self.pools;
  }
  ...
  function _createPool(
    ...
    ICommon.Mode mode, 
    uint poolId,
    function (uint, ICommon.FuncTag) internal callback
    ) private {
      bool(p1.duration > 0).assertChained3(p1.members.length > 0, p1.duration < type(uint8).max, "4");
      self.push();
      self[poolId].uints = ICommon.Uints(mode, p1.quorum, 0, p1.ccr, p1.duration * 1 days);
      self[poolId].uint256s =  ICommon.Uint256s(p1.value, 0, p1.value);
      self[poolId].addrs = ICommon.Addresses(p1.asset, address(0));
      callback(poolId, ICommon.FuncTag.ADD);
   ...
   */
  function _createPublicPool( 
    Data.amountExist storage self, 
    ICommon.P1 memory p1,
    function (address) internal _account,
    function (bool, address) internal _check
    ...
    returns (uint poolId)
  {
    ...
    self[p1.value].assertNotUnchained("1");
    ...
    _createPool(p1, ICommon.Mode(0), poolId);
    ...
    returns (uint poolId) 
  {
    ...
    poolId = _generateGroupIndex();
    _createPool(p1, ICommon.Mode(1), poolId);
  ...
  function _initializeAccount(
    ...
    poolId.safeInitializeInfo(
      ICommon.Info(_addNewMember(poolId, alc), 0, 0, 0, 0, 0, isAdmin, true, false), 
      lock,
      reduce,
      IAccount(alc),
      value,
  ...
  function _getMode(Data.pools storage self, uint poolId) internal view returns(ICommon.Mode mode) {
   ...
   */
  function _computePenalty(Data.pcd storage self, uint poolId, address user) internal view returns (uint256) {
  ...
  function updatePenFee(Data.pcd storage self, uint8 newRate) internal {
  ...
  */
  function _getFinance(
    ...
    function (address) internal _account,
    function () internal _getQFTPriceInETH
  ) 
    internal
    returns(address valueTo, uint position)
  {
    _lock(poolId, ICommon.FuncTag.GET);
    ICommon.Pool memory pool = _fetchPoolData(self.pools, poolId);
    ...
    (valueTo, position) = _updateMemberData(self, ICommon.UpdateParam(
    ...
    self.pools[poolId].allGh ++;
  }

  ///@dev Returns info if current beneficiary has defaulted otherwise an empty struct is returned.
  function _enquireLiquidation(Data.pools storage self, uint poolId) internal view returns (ICommon.Liquidation memory) {
  ...
  */
  ...
  {
    ICommon.Liquidation memory liq = _enquireLiquidation(self.pools, poolId);
    ...
    address token = _fetchPoolData(self, poolId).addrs.asset;
  ...
  */
    ...
    internal
  {
    ...
    emit ICommon.Cancellation(poolId, _p.uint256s.unit, alc);
  }
  ...
  */
    ...
    internal
  {
    ...
    emit ICommon.RoundUp(_poolId, pool);
  }

  function updatePublicData(
    Data.pcd storage self,
    address feeTo,
    address token,
    uint8 penFee,
    uint8 makerFee
    ...
    self = ICommon.PublicData(feeTo, token, penFee, makerFee);
  }
