    ...
    const scale = (
      baseCurrency === BaseCurrency.USD
        ...
        : baseCurrency === BaseCurrency.EUR
          ? 1.0 / (typeof(eurUsdPrice)=="number" ? eurUsdPrice : eurUsdPrice.value)
          : baseCurrency === BaseCurrency.ETH
            ? 1.0 / (typeof(ethUsdPrice)=="number" ? ethUsdPrice : ethUsdPrice.value)
            : 1.0 / (typeof(celoUsdPrice)=="number" ? celoUsdPrice : celoUsdPrice.value)
    );

    setBalance({
      CELO: {
        raw: celoAmount,
        base: ((typeof(celoUsdPrice)=="number" ? celoUsdPrice : celoUsdPrice.value) * (+celoAmount) * scale),
        exchange: (typeof(celoUsdPrice)=="number" ? celoUsdPrice : celoUsdPrice.value) * scale
      },
      cEUR: {
        raw: ceurAmount,
        base: ((typeof(eurUsdPrice)=="number" ? eurUsdPrice : eurUsdPrice.value) * (+ceurAmount) * scale),
        exchange: (typeof(eurUsdPrice)=="number" ? eurUsdPrice : eurUsdPrice.value) * scale
      },
      cUSD: {
        raw: cusdAmount,
