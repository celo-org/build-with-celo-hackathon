...
import * as string from '@ethersproject/strings'
...
export const shuffleArray = <T>(...arrayIn: T[] | T[][]): T[] => {
  const array: T[] =
    arrayIn.length === 1 && Array.isArray(arrayIn[0]) ? (arrayIn[0] as T[]) : (arrayIn as T[])
...
export const toUTF8String = (input: Uint8Array): string => {
  //let TextDecoderConstructor = (window || {}).TextDecoder || require('util').TextDecoder
  //const textDecoder = new TextDecoderConstructor()
  return string.toUtf8String(input)
}

export const fromUTF8String = (input: string): Uint8Array => {
  //let TextEncoderConstructor = (window || {}).TextEncoder || require('util').TextEncoder
  //const textEncoder = new TextEncoderConstructor()
  return string.toUtf8Bytes(input)
}
