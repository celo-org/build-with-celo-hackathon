  ...
  return (
    <div className="grid grid-cols-2 divide-x m-1">
      <div className=" bg-slate-300 h-60 rounded-2xl m-1 flex justify-start p-3">
        <div>
          <h2 className=" text-green-400 font-bold">cUSD Balance: {parseFloat((props.cUSD/10**18).toFixed(4))}</h2><br/>
          <h2 className=" font-bold text-orange-400">Rehema  Balance: {props.balance} </h2>
        </div>
         ...
         <div className="flex justify-around">
          ...
          <div className="mt-2">
            ...
            <input
                ...
                type="submit"
                name="Send Transcation"
                onClick={()=>{props.shareRehemaToken(address,amount)}}
              />
          </div>
         </div>
        </div>
